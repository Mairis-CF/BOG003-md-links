...
const mdLinks = (path, ...option) => {
 ...
 const validatedPath = solvePath(path)['absolutePath'];
      ...
      if (solvePath(path)['existPath']) {//si la ruta existe, determina las propiedades del segundo argumento, option
        if (option.length === 1 || option.length === 0) {
          //determina de que tipo ha sido el valor ingresado en option
            ...
            case 'number'|| 'string' || 'boolean' || 'function':
              return Promise.reject('El segundo argumento de la función mdLinks es invalido. Debe ser de tipo objeto booleano.');
            ...
            case 'object': {
              ...
              //tipo null
                  if (option[0] === null) {
                    return Promise.reject('No puede leerse argumento de tipo null.')
                  }

                  //objeto con más de una propiedad o vacío
                  if(Object.keys(option[0]).length === 0 || Object.keys(option[0]).length > 1) {
                    return Promise.reject('El segundo argumento de tipo objeto en la función mdLinks se encuentra vacío o tiene más de una propiedad.')
                  }
                  ...
                  if(Object.keys(option[0].length === 1 && Object.keys(option[0]))[0] === 'validate' && typeof option[0].validate !== 'boolean') {
                    return Promise.reject('El segundo argumento de tipo objeto en la función mdLinks, contiene la propiedad validate pero no es un booleano');
                  }
        ...
        return Promise.reject(new Error('La función mdLinks recibe un segundo argumento y es de tipo objeto booleano'));
      }
